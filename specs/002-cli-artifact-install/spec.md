...
- **FR-007**: The `--force` flag MUST cause the CLI to re-fetch the artifact bypassing the cache, and to proceed past conflict guards (foreign-file protection and locally-modified content). If the fetched content hash is identical to the installed content hash, the install MAY still report `SKIP_IDENTICAL` — `--force` guarantees a fresh fetch and removes write guards, not a forced overwrite when content is identical.
...
- **FR-008b**: **Symlink mode** is the default installation mode. On Windows, directory junctions MUST be used as the symlink equivalent. If junction/symlink creation fails for **any reason on any platform** (insufficient permissions, unsupported filesystem, container restrictions, etc.), the system MUST fall back to copy mode for the affected target and log a warning. The fallback is per-target: other targets in the same run are not affected. **Copy mode** (`--copy`) is opt-in and creates independent file copies at each target path with no canonical directory. Note: in copy mode, `copilot/project` writes to `.agents/skills/<name>/` — the same path that symlink mode uses as the canonical directory. This is a path coincidence (it is Copilot's read path), not a canonical directory; no symlinks are created.
...
- **FR-019**: The system MUST fetch a remote manifest from `aidriven-resources` that lists available artifact types, names, the path of the skill subdirectory within the repository archive, a per-skill `contentHash` (SHA-256 computed from sorted file contents of the skill directory — same algorithm as `computedHash` in the lockfile), and AI target compatibility metadata. The manifest does NOT carry the source commit SHA; that is resolved at runtime via the GitHub API (FR-022b). Skills are unversioned — the manifest always points to the latest content on the default branch.
...
- **FR-024**: After extracting the skill subdirectory from the tarball, the system MUST compute the SHA-256 content hash of the extracted files (sorted by relative path, same algorithm as `computedHash` in the lockfile) and verify it against the `contentHash` declared in the manifest entry. If verification fails, the artifact MUST NOT be installed and an integrity error MUST be reported. This approach is stable across GitHub tarball regenerations and consistent with the lockfile hash algorithm.
...
- **FR-032**: After successful installation, the system MUST write a lockfile entry for the installed skill. For project scope, the lockfile is `<project-root>/aidriven-lock.json`. For user scope, the lockfile path is OS-specific and co-located with the user cache root: `~/.cache/aidriven/install-records.json` on Linux/macOS; `%LOCALAPPDATA%\aidriven\install-records.json` on Windows (mirrors the cache directory convention). Path resolution MUST use the same helper as `_paths.py` uses for `cache_dir`. The lockfile MUST use the following design:
...
- **Lockfile**: A JSON file recording what was installed. Project scope: `<project-root>/aidriven-lock.json` (VCS-friendly — sorted keys, no timestamps). User scope: `~/.cache/aidriven/install-records.json` on Linux/macOS, `%LOCALAPPDATA%\aidriven\install-records.json` on Windows. Each entry tracks: source, source commit SHA, computed hash (SHA-256 of sorted file contents), AI targets, scope, install mode.
...
- Cache storage and the user-scope lockfile use the OS standard user cache directory (`~/.cache/aidriven/` on Linux/macOS, `%LOCALAPPDATA%\aidriven\` on Windows). The user lockfile (`install-records.json`) is co-located at the cache root, not inside the `cache/` subdirectory. No user configuration is required for these paths in this iteration.
